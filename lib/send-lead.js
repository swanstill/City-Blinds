...
/**
 * Logs the *shape* of the Twilio env vars (lengths, formats, whitespace)
 * without leaking the actual secret values. Used to diagnose "invalid
 * username" auth errors in production.
 */
function logEnvShape() {
  const sid = process.env.TWILIO_ACCOUNT_SID || '';
  const token = process.env.TWILIO_AUTH_TOKEN || '';
  const from = process.env.TWILIO_FROM_NUMBER || '';
  const to = process.env.BUSINESS_PHONE || '';
  const ws = function (s) { return /^\s/.test(s) ? 'leading' : /\s$/.test(s) ? 'trailing' : (/\s/.test(s) ? 'internal' : 'none'); };
  console.error(
    '[twilio-env] SID: len=' + sid.length +
    ' startsAC=' + /^AC/.test(sid) +
    ' hex32=' + (/^AC[0-9a-f]{32}$/i.test(sid)) +
    ' ws=' + ws(sid) +
    ' | TOKEN: len=' + token.length +
    ' hex32=' + (/^[0-9a-f]{32}$/i.test(token)) +
    ' ws=' + ws(token) +
    ' | FROM: len=' + from.length + ' ws=' + ws(from) +
    ' | TO: len=' + to.length + ' ws=' + ws(to)
  );
}
